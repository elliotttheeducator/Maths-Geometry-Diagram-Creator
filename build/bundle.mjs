...
// The instructions an AI assistant needs live in js/aiCard.js so the app can show them
// too; these two files are generated from it, so the three can never drift apart. One
// is for pasting into a Claude Project, the other for uploading as a Claude Skill.
const { AI_CARD } = await import(pathToFileURL(join(ROOT, "js/aiCard.js")).href);

writeFileSync(
  join(ROOT, "AI-INSTRUCTIONS.md"),
  `<!-- Generated by build/bundle.mjs from js/aiCard.js -- edit that, not this. -->\n\n${AI_CARD}`,
  "utf8"
);

const SKILL_DIR = join(ROOT, "skill", "geometry-diagrams");
...
console.log("Wrote AI-INSTRUCTIONS.md and skill/geometry-diagrams/SKILL.md");
